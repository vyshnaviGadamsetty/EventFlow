...
// import { Clock, MapPin, Repeat, Edit3, Trash2, Calendar } from 'lucide-react';
...
// const EventCard = ({ event, onEdit, onDelete, isDraggable = true, setIsHovering, isDarkMode, handleDragStart }) => {
//   const categories = [
...
//                 onEdit(event);
//               }}
//               className="p-1 bg-black/20 hover:bg-black/40 rounded text-black/80 hover:text-black transition-all duration-200"
//             >
...
//                 onDelete(event.id);
//               }}
//               className="p-1 bg-black/20 hover:bg-red-500 rounded text-black/80 hover:text-white transition-all duration-200"
//             >
...
import React from 'react';
import { Clock, MapPin, Repeat, Edit3, Trash2, Calendar, Briefcase, User, Heart, Coffee, Star } from 'lucide-react';
...
const EventCard = ({
  ...
  setIsHovering,
  isDarkMode,
  handleDragStart // <-- new prop for drag start handler
}) => {
  const categories = [
    { id: 'work', icon: Briefcase },
    { id: 'personal', icon: User },
  ...
  return (
    <div
      draggable={isDraggable}
      onDragStart={(e) => handleDragStart && handleDragStart(e, event)} // pass event object along with native event
      className={`${event.color} rounded-lg p-3 mb-2 cursor-move shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 group relative overflow-hidden`}
      onMouseEnter={() => setIsHovering && setIsHovering(true)}
      onMouseLeave={() => setIsHovering && setIsHovering(false)}
    >
